...
class VandermondeTransform:
    def __init__(self, kX, kY=None, dataset=None, dataClass='pic', dim=1):
        self.kX = kX
        self.kY = kY
        ...
        if dim == 1:
            ...
            self._X = torch.cat((torch.arange(self.kX, dtype=torch.float),
                                 torch.arange(start=-(self.kX), end=0, dtype=torch.float)), 
                                 axis=0)[None,:,None] # [1,2*kX,1]
            self._Y = torch.cat((torch.arange(self.kY, dtype=torch.float), 
                                 torch.arange(start=-(self.kY-1), end=0, dtype=torch.float)),
                                 axis=0)[None,:,None] # [1, 2*kY-1, 1]
    ...
    def make_1Dmatrix(self, x_data):
        ...
        if self.dataset is not None:
            xPos = torch.tensor(self.dataset.grid[0], dtype=torch.float32).repeat(x_data.shape[0],1)
        else:
            xPos = x_data.real

        # scaling btw 0 and 2*pi
        xPos = ((xPos - xPos.min()) / xPos.max()) * 2 * np.pi
        V = torch.exp(-1j * ((self._X - int(self.kX/2))* xPos[:, None, :])) # [1, kX, 1] x [B, 1, nX]
        self.V = V /np.sqrt(nParticle)  # normalization
    ...
    def make_2Dmatrix(self, x_data, y_data):
        ...
        if self.dataset is not None:
            xPos = torch.tensor(self.dataset.grid[0], dtype=torch.float32).repeat(x_data.shape[0],1)
            yPos = torch.tensor(self.dataset.grid[1], dtype=torch.float32).repeat(y_data.shape[0],1)
        else:
            xPos = x_data.real
            yPos = y_data.real
        ...
        self.forward_mat = torch.exp(-1j* (X_mat+Y_mat))/ xPos.shape[-1]
